...
install_cli_brew() {
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "Chora CLI already installed via Homebrew"
    else
      warn "Failed to install chora via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "Chora CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Chora CLI from GitHub Releases..."
  ...
  local url="https://github.com/Chora-Inc/Chora-Release/releases/download/${latest}/chora-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/chora.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/chora.tar.gz" -C "$tmp_dir" chora

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with CHORA_BIN_DIR.
  local bin_dir="${CHORA_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/chora" "$bin_dir/chora"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/chora" "$bin_dir/chora"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/chora" "$bin_dir/chora"
    chmod +x "$bin_dir/chora"
  ...
  rm -rf "$tmp_dir"
  ok "Chora CLI installed to $bin_dir/chora"
}
...
get_selfhost_ref() {
  if [ -n "${CHORA_SELFHOST_REF:-}" ]; then
    printf '%s' "$CHORA_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Chora CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Chora CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Chora CLI is already the latest version"
  fi
}
...
install_cli() {
  if command_exists chora; then
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Chora CLI is up to date ($current_ver)"
      return 0
    fi

    info "Chora CLI $current_ver installed, latest is $latest_ver — upgrading..."
  ...
  # Verify
  if ! command_exists chora; then
    fail "CLI installed but 'chora' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Chora self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  info "Pulling official Chora images..."
  pull_official_selfhost_images
  info "Starting Chora services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  Chora — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Chora CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Chora — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Chora server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}chora setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Chora installation found at $INSTALL_DIR"
  fi

  if command_exists chora; then
    chora daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Chora CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  CHORA_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.chora/server)"
        echo "  CHORA_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  CHORA_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'chora setup' to configure your environment."
        exit 0
        ;;
