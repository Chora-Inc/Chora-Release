# Chora installer for Windows — one command to get started.
#
# Install CLI (default): connects to chora.ai
#   irm https://raw.githubusercontent.com/Chora-Inc/Chora-Release/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Chora server + installs CLI + configures
#   $env:CHORA_MODE="local"; irm https://raw.githubusercontent.com/Chora-Inc/Chora-Release/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/Chora-Inc/chora.git"
$RepoWebUrl    = "https://github.com/Chora-Inc/chora"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".chora\server"
$InstallDir    = if ($env:CHORA_INSTALL_DIR) { $env:CHORA_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/Chora-Inc/Chora-Release/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:CHORA_SELFHOST_REF) {
        return $env:CHORA_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = chora version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Chora CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Chora requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/Chora-Inc/Chora-Release/releases/download/$latest/chora-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "chora-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "chora.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/Chora-Inc/Chora-Release/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "chora.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "chora-cli-$version-windows-$arch.zip"
        $legacyAsset = "chora_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "chora.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".chora\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "chora.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "chora.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "chora.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "chora.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Chora CLI installed to $binDir\chora.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "chora") {
        ...
        if ($isUpToDate) {
            Write-Ok "Chora CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Chora CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Chora CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "chora")) {
        Write-Fail "CLI installed but 'chora' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Chora self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:CHORA_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Chora server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Chora repository..."
    ...
    Write-Info "Pulling official Chora images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Chora services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Chora server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Chora - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Chora CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     chora setup               " -NoNewline; Write-Host "# Connect to Chora Cloud (chora.ai)" -ForegroundColor DarkGray
    Write-Host "     chora setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:CHORA_MODE="with-server"; irm https://raw.githubusercontent.com/Chora-Inc/Chora-Release/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Chora - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Chora server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     chora setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:CHORA_MODE="stop"; irm https://raw.githubusercontent.com/Chora-Inc/Chora-Release/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Chora services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Chora installation found at $InstallDir"
    }

    if (Test-CommandExists "chora") {
        try {
            chora daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:CHORA_MODE) { $env:CHORA_MODE.ToLower() } else { "default" }
